...
class GeminiService:
    ...
    def __get_prompt(self):
        """ Returns a prompt for the Gemini model"""
        return """
    ...
    def retrieve_history(self, db: Session, device_id: str):
        ...
        return db.scalars(select(HistoryItem)
                          ...
                          .where(HistoryItem.device_id == device_id)
                          .order_by(HistoryItem.timestamp.desc())).all()

    def predict_exercise(self, contents: bytes):
        """ Predict exercise information from an image using Gemini. """
        image_file = self.__process_image(contents)
